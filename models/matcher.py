...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            tgt_ids = torch.cat([v["labels"] for v in targets])  # [\sum_{i=0}^{bs-1} num_target_boxes_i,]
            tgt_bbox = torch.cat([v["boxes"] for v in targets])  # [\sum_{i=0}^{bs-1} num_target_boxes_i, 4]
            ...
            cost_giou = -generalized_box_iou(box_cxcywh_to_xyxy(out_bbox),
                                             box_cxcywh_to_xyxy(tgt_bbox))  # [bs * num_queries, \sum_{i=0}^{bs-1} num_target_boxes_i]
